...
const Sidebar = ({ setTitle, feedback }: SidebarProps) => {
  ...
  const handleItemClick = (itemTitle: string, itemCaminhoPasta?: string) => {
    ...
    if (itemTitle !== "Logs" && itemCaminhoPasta) {
      ...
      <nav className="flex-grow mt-4 space-y-2 px-4">
        ...
        <button
          ...
          className={`w-full text-left px-3 py-2 rounded ${
            activeItem === "Logs" ? "bg-rotion-700" : "hover:bg-rotion-700"
          }`}
        >
          Logs
        </button>
...
const MenuItem = ({ label, isActive, onClick, showFeedback }: MenuProps) => (
  <div
    onClick={onClick}
    className={`sidebar-item 
                        ...
                        cursor-pointer 
                        transition-all 
                        ${isActive ? "is-active bg-rotion-500 text-white font-semibold" : "hover:bg-rotion-700 hover:text-rotion-50"}`}
  >
